...
board=numpy.ndarray((int(screen_width/20),int(screen_height/20)),dtype=object)
dfs=numpy.ndarray((int(screen_width/20),int(screen_height/20)),dtype=object)
...
def initialize_board():  
	for x in range(0,int(screen_width/20)):
		for y in range(0,int(screen_height/20)):
			board[x][y]=array()
...
class blueglider:           
	...
	def __init__(self):     #initiliazing the glider
		...
		display.flip()
		board[int(self.pos_x/20)][int(self.pos_y/20)].value=1
		self.pos_x=(int(screen_width/4))+10
...
class greenglider:  #same for green glider
	...
	def __init__(self):
		...
		display.flip()
		board[int(self.pos_x/20)][int(self.pos_y/20)].value=1
		self.pos_x=int((3*screen_width)/4)+10
...
def gameplay(object1,object2,move1,move2):
	...
	if board[int(object1.pos_x/20)][int(object1.pos_y/20)].art=="no":
		
		object1.art_check=0
		components[board[int(object1.pos_x/20)][int(object1.pos_y/20)].comp] -= 1
	...
	if board[int(x/20)][int(y/20)].art=="no":
		
		components[board[int(x/20)][int(y/20)].comp] -= 1
		calc_artpoints()
...
def artpoint_reset():
	for i in range(0,int(screen_width/20)):
		for j in range(0,int(screen_height/20)):
			board[i][j].art="no"
...
def dfs_graph(x,y):
	...
	for i in range(0,4):
		if (x+dx[i]>=0 and x+dx[i]<int(screen_width/20)) and (y+dy[i]>=0 and y+dy[i]<int(screen_height/20)):
...
def voronoi_diagram(x1,y1,x2,y2):
	...
	coordi2.pos_x=x2
	coordi2.pos_y=y2
	queue.Queue().put(coordi1)
	queue.Queue().put(coordi2)
	count1=1
	count2=1
	board[x1][y1].value=1
	board[x2][y2].value=1

	while (not queue.Queue().empty() )or (not queue.Queue().empty()):  #until all squares are evaluated
		for i in range(0,count1):
			temp=queue.Queue().get()
			count1-=1
			...
			if board[temp.pos_x][temp.pos_y].color =="none" or board[temp.pos_x][temp.pos_y].color =="blue":
				for i in range(0,4):
					if temp.pos_x+dx[i]>=0 and temp.pos_x+dx[i]<int(screen_width/20) and temp.pos_y+dy[i]>=0 and temp.pos_y+dy[i]<int(screen_height/20):
...
def minimax_move(object1,object2): 
	x1=int(object1.pos_x/20)
	y1=int(object1.pos_y/20)
	x2=int(object2.pos_x/20)
	y2=int(object2.pos_y/20)
	node=minimax_node()
		...
		if level%2==0:  #max node
			...
			for i in range(0,4):
				...
				if node.alpha<=node.beta:	
					...
					if x1+dx[i]<0 or x1+dx[i]>=int(screen_width/20) or y1+dy[i]<0 or y1+dy[i]>=int(screen_height/20):
		...
		if level%2==1 :   #min node
			for i in range(0,4):
				...
				if node.alpha<=node.beta:	
					...
					if x2+dx[i]<0 or x2+dx[i]>=int(screen_width/20) or y2+dy[i]<0 or y2+dy[i]>=int(screen_height/20):
...
def huggwall(object,pre_move):
	x=int(object.pos_x/20)
	y=int(object.pos_y/20)
	max=-1
		...
		if board[x][y-1].value==0:
			...
			if max<count and board[x][y-1].art=="no":
				...
				if art_max<components[board[x][y-1].comp]:
					...
					art_move=up
		#down
	if pre_move!=3-down and y+1<int(screen_height/20):
		if board[x][y+1].value==0 :
			count=0
			count=count_neighbours(x,y+1)
			if max<count and board[x][y+1].art=="no":
		...
		if board[x-1][y].value==0:
			...
			if max<count and board[x-1][y].art=="no":
				...
				if art_max<components[board[x-1][y].comp]:
					...
					art_move=left
		#right
	if pre_move!=3-right and x+1<int(screen_width/20):
		if board[x+1][y].value==0:	
			count=0
			count=count_neighbours(x+1,y)
			if max<count and board[x+1][y].art=="no":
...
def count_neighbours(x,y):
	count=0
	if x-1>=0 and y-1>=0 and x+1<int(screen_width/20) and y+1<int(screen_height/20):
...
def checkcollision(object,move):   #check whether a bot has crashed into a wall
	if(move==up):
		if object.pos_y-20<0:
			return true
		elif board[int(object.pos_x/20)][int(object.pos_y/20)-1].value==1:
			return true
		else:
			return not true
	if(move==down):
	...
	if(move==left):
		if object.pos_x-20<0:
			return true
		elif board[int(object.pos_x/20-1)][int(object.pos_y/20)].value==1:
			return true
		else:
			return not true
	if(move==right):
...
def update_board(object): #update the board and object upon movement
	board[int(object.pos_x/20)][int(object.pos_y/20)].value=1
	object.comp=board[int(object.pos_x/20)][int(object.pos_y/20)].comp
